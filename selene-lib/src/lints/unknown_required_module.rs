        ...
        visitor
            ...
            .map(|(start, end, module_path, path_checked)| {
                Diagnostic::new(
                    ...
                    format!("could not find required module '{}' (checked path: {})", 
    ...
    fn replace_wait_for_child_calls(&self, input: &str) -> String {
        ...
        while let Some(pos) = processed_string[search_pos..].find(":WaitForChild(") {
            ...
            if let Some(close_pos) = processed_string[real_pos..].find(')') {
                ...
                let inner_content = &processed_string[real_pos + ":WaitForChild(".len()..close_real_pos];
...
impl<'a> Visitor for UnknownRequiredModuleVisitor<'a> {
    ...
    fn visit_local_assignment(&mut self, assignment: &ast::LocalAssignment) {
        if let Some((name, expr)) = assignment.names().iter().next().zip(assignment.expressions().iter().next()) {
            ...
            self.local_vars.insert(var_name.trim().to_string(), expr_str);
        }
    }
                ...
                if let Some(ast::Suffix::Call(ast::Call::AnonymousCall(
                    ...
                    if arguments.len() == 1 {
                        ...
                        if let Some(arg) = args_iter.next() {
                            ...
                            let first_part = arg_string.split(|c: char| c == '.' || c == ':').next().unwrap();
                            potential_logs.push(format!("First part: {}", first_part));
                            if first_part == "ServerScriptService" || first_part == "ReplicatedStorage" {
                                if let Some(root_path) = self.root_path.as_ref() {
                                    arg_string = format!("{}{}", root_path.to_string_lossy(), arg_string);
                                } else {
                                    potential_logs.push(format!("No root path found :("));
                                }
                            } else {
                                let variable_value = self.local_vars.get(first_part);
                                potential_logs.push(format!("Checking for variable in scope: {}", first_part));
                                if let Some(variable_value) = variable_value {
                                    potential_logs.push(format!("Found variable in scope: {} -> {}", first_part, variable_value));
                                    // replace the first part with the variable's value
                                    arg_string = arg_string.replace(first_part, &variable_value.to_string());
                                }
                            }
                            ...
                            if let Some(remainder) = arg_string.strip_prefix("script.Parent.") {
                                ...
                                let current_file_path = self.current_file.as_ref().unwrap().parent().unwrap();
                                arg_string = format!("{}.{}", current_file_path.to_string_lossy(), remainder);
                            }
                            ...
                            if arg_string.starts_with("ReplicatedStorage.Packages") || arg_string.starts_with("ServerStorage.SoftRequire") {
                                return;
                            }
                            ...
                            potential_logs.push(format!("lua_script_path: {}", lua_script_path.to_string_lossy()));
                            ...
                            if !found_file {
                                let init_lua_path = Path::new(&arg_string).with_file_name("init.lua");
                                potential_logs.push(format!("Failed to find lua, checking for folder/init.lua: {}", init_lua_path.to_string_lossy()));
                            ...
                            if !found_file {
                                potential_logs.push(format!("Failed to find file for: {}", arg.to_string()));
                                potential_logs.push(format!("Converted path: {}", arg_string));
                                potential_logs.push(format!("Current file: {}", self.current_file.as_ref().unwrap().to_string_lossy()));
                            }
                            potential_logs.push(format!("--------------------------------"));
